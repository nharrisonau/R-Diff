...
This backlog expands the benchmark with samples that remain consistent with the current dataset:
...
3. Target is added to `targets/baselines_config.json` with immediate prior baseline.
...
8. `[ ]` `targets/baselines_config.json` entry reviewed (mode, current_version, immediate baseline).
