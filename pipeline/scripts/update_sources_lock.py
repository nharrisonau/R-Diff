...
def _source_relpath(cfg: dict[str, object], role: str) -> str:
    override = str(cfg.get(f"source_{role}", "") or "").strip()
    if override:
        return f"targets/{override}"
    return f"targets/{cfg['path']}/{role}"
