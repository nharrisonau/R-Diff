...
09. Push the commit and the changes.
10. Tag and push the Docker image: `docker tag rosarum:X.Y.Z plumtrie/rosarum:X.Y.Z`,
    `docker push plumtrie/rosarum:X.Y.Z`.
11. Tag and push the new image as "latest":
    `docker tag plumtrie/rosarum:X.Y.Z plumtrie/rosarum:latest`,
    `docker push plumtrie/rosarum:latest`.
12. Prepare for the next version by bumping the PATCH number in the version and appending `"-dev"`.
