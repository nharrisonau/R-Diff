...
02. Update `VERSION`; this should simply be removing the `"-dev"` part.
03. Build the entire benchmark with `make -C targets`. It should succeed. If you run it again, it
...
04. Update `CHANGELOG.md`.
05. Build the Docker image with the `build.sh` script and make sure it succeeds (check that an image
    with the right version has been created with `docker images`).
06. Run the Docker image with the `run.sh` script and inspect the contents of the container.
...
08. Push the commit and the changes.
09. Tag and push the Docker image: `docker tag rosarum:X.Y.Z plumtrie/rosarum:X.Y.Z`,
    `docker push plumtrie/rosarum:X.Y.Z`.
10. Tag and push the new image as "latest":
