...
static int  getFlashCfg(int eid, webs_t wp, int argc, char_t **argv);
 ...
 * description: parse values delimited by semicolon, and return the value "offset of memory"
 *              according to given index (starts from 0)
 * MARK: the original values will not be destroyed by strtok
 * WARNING: offset will over the values's buffer range in some case
 */
char *getNthValueOffset(int index, char *values)
{
	int i;
	char *offset;
	char *preOffset = NULL;
	...
	if (NULL == values)
		return NULL;
	preOffset = values;
	for (i = 0, offset = strstr(values, ";");
			(i < index) && offset;
			i++, offset = strstr(offset, ";")) {
		preOffset = ++offset;
	}
	return preOffset;
}
...
void formDefineUtilities(void)
{
	...
	websAspDefine(T("getFlashCfg"), getFlashCfg);
