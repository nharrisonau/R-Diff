...
static void nvramIrqHandler(int signum)
{
	...
	system("iwpriv ra0 set WscGetConf=1");
		...
		if (NULL == fp) {
			printf("wpsledpbc: goAhead is not running\n");
			return;
		}
		fscanf(fp, "%d", &gopid);
		if (gopid < 2) {
			printf("wpsledpbc: goAhead pid(%d) <= 1\n", gopid);
			return;
		}
...
int main(int argc, char *argv[])
{
	...
	signal(SIGUSR1, nvramIrqHandler); //shout press
	signal(SIGUSR2, nvramIrqHandler); //long press
