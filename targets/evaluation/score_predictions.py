...
VALID_TRACKS = {"malicious", "benign"}
VALID_MISSING_POLICIES = {"error", "negative", "ignore"}
BUCKET_ORDER = ("major", "minor", "patch", "build", "other")
...
def _repo_root_from_script() -> Path:
    # targets/evaluation/score_predictions.py -> parents[2] is repo root
...
def _load_list_baselines_fn(repo_root: Path) -> Callable[..., list[tuple[str, str]]]:
    malicious_scripts = repo_root / "targets" / "malicious" / "scripts"
    if not malicious_scripts.exists():
        raise RuntimeError(f"missing malicious scripts directory: {malicious_scripts}")
    if str(malicious_scripts) not in sys.path:
        sys.path.insert(0, str(malicious_scripts))
    try:
...
def _malicious_units_from_config(
    *,
    repo_root: Path,
    ...
    for entry in entries:
        ...
        if mode == "manual":
            ...
            raise ValueError(f"unknown malicious mode for {target}: {mode!r}")

        for baseline_version in baseline_versions:
            unit_id = f"malicious::{group}::{target}::{baseline_version}"
            if unit_id in seen:
                continue
            seen.add(unit_id)
            units.append(
...
def _benign_units_from_pairs_binned(pairs_binned: Path) -> list[EvalUnit]:
    if not pairs_binned.exists():
        raise FileNotFoundError(f"missing benign pairs_binned.csv: {pairs_binned}")

    units: list[EvalUnit] = []
    seen: set[str] = set()
    with pairs_binned.open(newline="") as fh:
        ...
        for row in reader:
            ...
            if not product or not prev_version or not next_version:
                continue
            unit_id = f"benign::{product}::{prev_version}::{next_version}"
            if unit_id in seen:
                continue
            seen.add(unit_id)
            units.append(
    ...
    with path.open(newline="") as fh:
        ...
        for row in reader:
            ...
            if track not in VALID_TRACKS:
                raise ValueError(f"invalid track in ground truth for {unit_id}: {track!r}")
...
def _build_ground_truth_units(
    *,
    ...
    malicious = _malicious_units_from_config(repo_root=repo_root, baselines_config=baselines_config)
    benign = _benign_units_from_pairs_binned(pairs_binned)
    units = malicious + benign
...
def _compute_metrics(rows: list[dict[str, object]], missing_count: int, ignored_count: int) -> dict:
    ...
    global_metrics = {
        ...
        if recall is None or tnr is None
        else (recall + tnr) / 2.0,
        "mcc": _mcc(tp, fp, tn, fn),
    }

    malicious_rows = [r for r in rows if r["track"] == "malicious"]
    benign_rows = [r for r in rows if r["track"] == "benign"]

    malicious_detected = sum(1 for r in malicious_rows if r["predicted_flagged"])
    malicious_total = len(malicious_rows)
    malicious_summary = {
        "total_units": malicious_total,
        "detected_units": malicious_detected,
        "missed_units": malicious_total - malicious_detected,
        "unit_recall": _safe_div(malicious_detected, malicious_total),
        "unit_miss_rate": _safe_div(malicious_total - malicious_detected, malicious_total),
    }

    by_group: dict[str, list[dict[str, object]]] = defaultdict(list)
    for row in malicious_rows:
        by_group[str(row.get("target_group", ""))].append(row)
    malicious_summary["group_recall"] = {
        group: {
    ...
    if target_totals:
        ...
        malicious_summary["target_recall_any"] = detected_targets / target_count
        malicious_summary["target_recall_full"] = full_targets / target_count
        malicious_summary["mean_target_baseline_recall"] = mean_target_recall
    else:
        malicious_summary["target_recall_any"] = None
        malicious_summary["target_recall_full"] = None
        malicious_summary["mean_target_baseline_recall"] = None

    scope_rows: dict[str, list[dict[str, object]]] = defaultdict(list)
    for row in benign_rows:
        scope = str(row.get("scope", "")).strip() or "unknown"
        scope_rows[scope].append(row)

    bucket_stats: dict[str, dict[str, float | int | None]] = {}
    bucket_rates: list[tuple[str, float]] = []
    scope_order = list(BUCKET_ORDER) + sorted(
        scope for scope in scope_rows.keys() if scope not in BUCKET_ORDER
    )
    for scope in scope_order:
        rows_for_scope = scope_rows.get(scope, [])
        if not rows_for_scope:
            continue
        fp_count = sum(1 for r in rows_for_scope if r["predicted_flagged"])
        total = len(rows_for_scope)
        tn_count = total - fp_count
        fp_rate = _safe_div(fp_count, total)
        bucket_stats[scope] = {
            "total_pairs": total,
            "false_positives": fp_count,
            "true_negatives": tn_count,
            "fp_rate": fp_rate,
            "false_alarms_per_100_pairs": None if total == 0 else 100.0 * fp_count / total,
        }
        if fp_rate is not None:
            bucket_rates.append((scope, fp_rate))

    bucket_macro_fp_rate = None
    worst_bucket = ""
    worst_bucket_fp_rate = None
    if bucket_rates:
        bucket_macro_fp_rate = sum(rate for _scope, rate in bucket_rates) / len(bucket_rates)
        worst_bucket, worst_bucket_fp_rate = max(bucket_rates, key=lambda item: item[1])

    benign_total = len(benign_rows)
    benign_fp = sum(1 for r in benign_rows if r["predicted_flagged"])
    benign_summary = {
        "bucket_fp": bucket_stats,
        "bucket_macro_fp_rate": bucket_macro_fp_rate,
        "worst_bucket": worst_bucket,
        "worst_bucket_fp_rate": worst_bucket_fp_rate,
        # Keep aggregate benign metrics as secondary context for compatibility.
        "overall": {
            "total_pairs": benign_total,
            "false_positives": benign_fp,
            "true_negatives": benign_total - benign_fp,
            "fp_rate": _safe_div(benign_fp, benign_total),
            "false_alarms_per_100_pairs": None
            if benign_total == 0
            else 100.0 * benign_fp / benign_total,
        },
    }
...
def _print_summary(metrics: dict) -> None:
    ...
    print(
        f"- Malicious: unit_recall={m['unit_recall']:.4f} "
        ...
        else "- Malicious: no scored malicious units"
    )
    bucket_fp = b.get("bucket_fp", {})
    if not bucket_fp:
        print("- Benign: no scored benign units")
        return

    bucket_parts: list[str] = []
    for scope in BUCKET_ORDER:
        entry = bucket_fp.get(scope)
        if not entry:
            continue
        rate = entry.get("fp_rate")
        fp = entry.get("false_positives")
        total = entry.get("total_pairs")
        if rate is None:
            continue
        bucket_parts.append(f"{scope}={rate:.4f} ({fp}/{total})")
    extra_scopes = sorted(scope for scope in bucket_fp if scope not in BUCKET_ORDER)
    ...
    if bucket_parts:
        print(f"- Benign bucket FP rates: {', '.join(bucket_parts)}")
    else:
        print("- Benign bucket FP rates: unavailable")

    macro = b.get("bucket_macro_fp_rate")
    worst_bucket = b.get("worst_bucket")
    worst_rate = b.get("worst_bucket_fp_rate")
    if macro is not None and worst_bucket and worst_rate is not None:
        print(
            f"- Benign bucket summary: macro_fp_rate={macro:.4f} "
            f"worst_bucket={worst_bucket} ({worst_rate:.4f})"
        )
...
def main() -> int:
    ...
    ap = argparse.ArgumentParser(
        description=(
            "Score detector predictions on R-Diff malicious and benign units. "
    ...
    ap.add_argument(
        "--baselines-config",
        default="",
        help="Path to malicious baselines_config.json (default: targets/malicious/baselines_config.json)",
    )
    ap.add_argument(
        "--pairs-binned",
        default="",
        help="Path to benign pairs_binned.csv (default: targets/benign/pairs_binned.csv)",
    )
    ap.add_argument(
        "--template-out",
        default="",
    ...
    pairs_binned = (
        Path(args.pairs_binned).resolve()
        if args.pairs_binned
        else (repo_root / "targets" / "benign" / "pairs_binned.csv")
    )
